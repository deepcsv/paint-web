...
export function scheduleStateWrite(stateJson) {
    pendingStateJson = stateJson;
    if (stateWriteScheduled)
        return;
...
export async function saveSnapshotPng(name, png) {
    await ensureDataDir();
...
export async function loadSnapshotPng(name) {
    try {
        const safe = sanitizeSnapshotName(name);
